...
const DREAM_FONT = '14px "IBM Plex Mono", monospace';
const DREAM_LINE_H = 26;
...
// ── Colours ──
const COL_TEXT     = '#c0bcb8';
const COL_DREAM    = '#7a7068';
const COL_SPEAKER  = '#7a7570';
const COL_PROMPT   = '#4a4540';
const COL_BG_START = 'rgba(10,9,11,0.92)';
const COL_BG_END   = 'rgba(10,9,11,0)';

export class NarrativeController {
  constructor(bus) {
    this.bus = bus;

    // Choices remain as DOM (accessibility: focus, keyboard, screen readers)
  ...
  goNode(id) {
    ...
    this._isDream    = node.voice === 'dream';
  ...
  // ── Draw narrative text onto Canvas ──
  draw(ctx) {
    if (!this.active || !this.displayText) return;

    const font    = this._isDream ? DREAM_FONT : NARRATOR_FONT;
    const lineH   = this._isDream ? DREAM_LINE_H : NARRATOR_LINE_H;
    const textCol = this._isDream ? COL_DREAM : COL_TEXT;

    // ── Prepare & layout the revealed text via Pretext ──
    ctx.font = font;
  ...
  // ── Show choice buttons (DOM — for accessibility) ──
